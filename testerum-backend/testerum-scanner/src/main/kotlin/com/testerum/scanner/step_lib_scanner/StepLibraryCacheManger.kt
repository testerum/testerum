...
class StepLibraryCacheManger(private val threadPool: ExecutorService) {

    companion object {
        private val LOGGER = LoggerFactory.getLogger(StepLibraryCacheManger::class.java)
    ...
    fun scan(jars: List<Path>, cacheFile: Path): ScannerBasicStepScanResult {
        LOGGER.info("scanning ${jars.size} jars...")
        ...
        val jarUrls: Array<URL> = jars
                ...
                .map { it.toUri().toURL() }
                .toTypedArray()
        val classLoader: ClassLoader = URLClassLoader(jarUrls, StepLibraryCacheManger::class.java.classLoader)
        ...
        val scanResult = ScannerBasicStepScanResult(libraries)

        if (changed) {
            serializeCache(scanResult, cacheFile)
        }

        LOGGER.info("scanning ${jars.size} jars took ${timeTakenMillis}ms")
    ...
    private fun parseExistingCache(cacheFile: Path): Map<String, ScannerBasicStepLibrary> {
        if (!Files.exists(cacheFile)) {
            return emptyMap()
        }

        val scanResult: ScannerBasicStepScanResult
        try {
        ...
        for (library in scanResult.libraries) {
            ...
            if (previousValue != null) {
                LOGGER.warn("detected duplicate library file [${library.jarFile.name}] in in cache at [${cacheFile.toAbsolutePath()}]; the jar file will be re-scanned")
    ...
    private fun tryToScanJar(jar: Path,
                             alreadyScannedFiles: Map<String, ScannerBasicStepLibrary>,
                             classLoader: ClassLoader): StepLibraryResult {
    ...
    private fun getLibraryFile(jar: Path): ScannerLibraryFile {
    ...
    private fun scanForAnnotations(jar: Path, classLoader: ClassLoader): AnnotationScanResult {
    ...
    private fun serializeCache(scanResult: ScannerBasicStepScanResult, destinationCacheFile: Path) {
    ...
    private fun getParamDescription(paramAnnotation: Param?): String? {
        ...
        val description: String = paramAnnotation.description
    ...
    private class StepPhaseAnnotation(annotation: Annotation) {
        ...
        private val _description: String? = when (annotation) {
            is Given -> if (annotation.description == "") null else annotation.description
            is When  -> if (annotation.description == "") null else annotation.description
    ...
    private class HookPhaseAnnotation(annotation: Annotation) {
        ...
        private val _description: String = when (annotation) {
            is BeforeAllTests -> annotation.description
            is BeforeEachTest -> annotation.description
            is AfterEachTest  -> annotation.description
            is AfterAllTests  -> annotation.description
            else     -> throw IllegalArgumentException("unsupported annotation [${annotation.annotationClass}]")
        }
