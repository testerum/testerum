...
object ChromeDriverBinaryClasspathLocationFinder {
    ...
    fun getClasspathLocationForCurrentOperatingSystem(): String {
        ...
        if (isWindows) {
            return BASE_PATH + "/windows/chromedriver_win32_v74.0.3729.6.exe"
        } else if (isLinux) {
            if (is64BitArchitecture) {
                return BASE_PATH + "/linux/chromedriver_linux64_v74.0.3729.6"
            } else {
            ...
            if (is64BitArchitecture) {
                return BASE_PATH + "/mac/chromedriver_mac64_v74.0.3729.6"
            } else {
                throw IllegalArgumentException(
